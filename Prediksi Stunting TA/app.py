...
if os.path.exists(model_path):
    ...
    model = None
    st.warning("⚠️ File model tidak ditemukan!")
...
if menu == "📊 Dashboard":
    ...
    if df.empty:
        ...
        if "Daerah" in df.columns and "Stunting" in df.columns:
            st.subheader("📍 Jumlah Kasus Stunting per Daerah")
            fig1 = px.bar(
                df.groupby("Daerah")["Stunting"].sum().reset_index(),
                x="Daerah", y="Stunting",
            ...
            st.warning("Kolom 'Daerah' atau 'Stunting' tidak ditemukan di dataset.")
...
# ============================
    ...
    if model is None:
        ...
        # Mapping categorical ke numerik
